    ...
    public function uploadtestAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                $fileUrl = $_SERVER['DOCUMENT_ROOT'].'/var/www/html/public/uploads/' . date("d-m-Y", time()) . '/' . $newName;
                if (!file_exists($_SERVER['DOCUMENT_ROOT'].'/var/www/html/public/uploads/' . date("d-m-Y", time()))) {
                    mkdir($_SERVER['DOCUMENT_ROOT'].'/var/www/html/public/uploads/' . date("d-m-Y", time()), 0700);
                }
                $form->file->addFilter('Rename', realpath(dirname('.')) .
                    DIRECTORY_SEPARATOR .
                    $_SERVER['DOCUMENT_ROOT'].'/var/www/html/public/uploads/' . date("d-m-Y", time()) . '/' .
                    DIRECTORY_SEPARATOR .
                    $newName);
                $form->file->receive();

                $xlsData = $this->getXLS($fileUrl);
